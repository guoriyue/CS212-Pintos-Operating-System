...
/* Array of lists with different priorities for the advanced scheduler. */
static struct list mlfqs_lists[PRI_MAX]; 
...
thread_init (void) 
{
  ...
  if (thread_mlfqs){
    ...
    initial_thread->priority = PRI_MAX - (initial_thread->recent_cpu / 4) - (initial_thread->nice * 2);
